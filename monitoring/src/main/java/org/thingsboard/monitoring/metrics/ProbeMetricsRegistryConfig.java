...
public class ProbeMetricsRegistryConfig {
    ...
    private OtlpMeterRegistry createOtlpRegistry(String endpoint, long stepMs, boolean alertingEnabled, MonitoringReporter reporter) {
        ...
        // same default sender OtlpMeterRegistry's own convenience constructor would have used -
        // going through the builder only to be able to wrap it for alerting below
    ...
    // reports the *actual* result of each OTLP push (real HTTP call, real response status) via the
    // same Slack/incident alerting transports/login already use - deliberately not a probe metric,
    // since a broken metrics pipe can't be expected to report its own breakage
